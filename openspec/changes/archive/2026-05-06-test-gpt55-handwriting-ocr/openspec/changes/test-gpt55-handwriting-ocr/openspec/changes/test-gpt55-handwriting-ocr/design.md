...
- What is GPT-5.5's official API pricing? (Need to update cost calculations when announced)
- Should we test GPT-5.5-mini if available, or only flagship GPT-5.5?
- What's the exact model ID for GPT-5.5 in OpenAI's API? (gpt-5.5, gpt-5.5-turbo, something else?)
